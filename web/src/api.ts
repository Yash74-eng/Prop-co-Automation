...
export const api = {
  health: () =>
    fetch('/api/health').then(
      handle<{ ok: boolean; anthropicKey: boolean; bizfileEnabled: boolean; model: string }>,
    ),

  upload: (file: File) => {
    const form = new FormData();
    form.append('file', file);
    return fetch('/api/jobs', { method: 'POST', body: form }).then(handle<JobSummary>);
  },
  ...
  uploadComps: (id: string, file: File, sheetName?: string) => {
    ...
    return fetch(`/api/jobs/${id}/comps`, { method: 'POST', body: form }).then(handle<JobSummary>);
  },

  uploadSuppression: (id: string, file: File) => {
    const form = new FormData();
    form.append('file', file);
    return fetch(`/api/jobs/${id}/suppression`, { method: 'POST', body: form }).then(
      handle<JobSummary>,
    );
  },
  ...
  exclusions: (id: string) =>
    fetch(`/api/jobs/${id}/exclusions?limit=500`).then(
      handle<{
        total: number;
  ...
  bizfile: (id: string, file?: File) => {
    ...
    return fetch(`/api/jobs/${id}/bizfile`, { method: 'POST', body: form }).then(
      handle<JobSummary & { rows: Record<string, unknown>[] }>,
    );
  },

  crossCheck: (id: string, body: Record<string, unknown>) =>
    fetch(`/api/jobs/${id}/cross-check`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify(body),
    }).then(handle<JobSummary & { findings: Record<string, unknown>[] }>),

  mailmerge: (id: string, file: File, splitPerRecord: boolean) => {
    const form = new FormData();
    form.append('file', file);
    form.append('splitPerRecord', String(splitPerRecord));
    return fetch(`/api/jobs/${id}/mailmerge`, { method: 'POST', body: form }).then(
      handle<{
        check: {
